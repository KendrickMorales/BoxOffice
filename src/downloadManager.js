    ...
    try {
      ...
      if (hasMagnet) {
        ...
        try {
          ...
          if (addError.code === 'ERR_INVALID_PROTOCOL' || addError.message.includes('Protocol')) {
            throw new Error('WebTorrent encountered an invalid protocol. This magnet link may be incompatible. Consider using qBittorrent.');
          }
          throw new Error(`Failed to add torrent: ${addError.message}`);
        }
      } else if (hasTorrentUrl) {
        console.log(`Downloading torrent file from: ${actualTorrentUrl.substring(0, 100)}...`);
        ...
        try {
          ...
          if (linkParam && linkParam.startsWith('magnet:?')) {
            ...
            console.log('Found magnet link in Prowlarr URL parameter, using it directly...');
          }
        } catch (urlParseError) {
          // URL parsing failed, continue with download
          console.log('Could not parse URL for magnet link, will try download...');
        }
        ...
        if (magnetFromUrl) {
          try {
            ...
            console.error('Error adding magnet link from URL parameter:', addError);
          ...
          const headers = {};
          if (actualTorrentUrl.includes('localhost:9696') || actualTorrentUrl.includes('prowlarr')) {
          ...
          try {
            ...
            try {
              ...
              try {
                ...
                if (noRedirectError.response && noRedirectError.response.status >= 300 && noRedirectError.response.status < 400) {
            ...
            if (redirectLocation && redirectLocation.startsWith('magnet:?')) {
              console.log('Prowlarr redirected to magnet link, using it directly...');
              try {
            ...
            if (downloadError.message.includes('magnet:') || downloadError.message.includes('Unsupported protocol')) {
              console.log('Detected magnet redirect error, attempting to extract magnet link...');
              ...
              if (linkParam && linkParam.startsWith('magnet:?')) {
                ...
                throw new Error(`Prowlarr redirected to magnet link but couldn't extract it. Error: ${downloadError.message}`);
              }
            } else {
              throw downloadError;
            }
        ...
        if (magnet && !hasMagnet && !actualTorrentUrl) {
          throw new Error(`Invalid magnet link format. Got: ${magnet.substring(0, 100)}...`);
        }
        if (torrentUrl && !hasTorrentUrl && !actualMagnet) {
          throw new Error(`Invalid torrent URL format. Must be http:// or https://. Got: ${torrentUrl ? torrentUrl.substring(0, 100) : 'undefined'}...`);
        }
      ...
      torrent.on('error', (error) => {
        ...
        if (error.code === 'ERR_INVALID_PROTOCOL' || error.message.includes('Protocol')) {
          downloadInfo.status = 'error';
          downloadInfo.error = 'Invalid torrent link format. This may be a compatibility issue with WebTorrent. Try using qBittorrent or another torrent client.';
        } else {
          downloadInfo.status = 'error';
    ...
    try {
      ...
      try {
        ...
        if (loginError.response && loginError.response.status === 401) {
          throw new Error('qBittorrent authentication failed. Check your username and password in .env file.');
        }
        throw new Error(`Cannot connect to qBittorrent at ${qbUrl}. Make sure qBittorrent is running and Web UI is enabled.`);
      }

      if (loginResponse.data !== 'Ok.') {
        throw new Error('qBittorrent authentication failed. Check your username and password.');
      }
      ...
      if (hasMagnet) {
        console.log(`Adding magnet to qBittorrent: ${actualMagnet.substring(0, 100)}...`);
        await axios.post(
          `${qbUrl}/api/v2/torrents/add`,
          null,
          {
            params: { 
              urls: actualMagnet,
      ...
      return { success: true, message: 'Download added to qBittorrent successfully' };
    } catch (error) {
      console.error('qBittorrent error:', error.message);
      if (error.response) {
        throw new Error(`qBittorrent API error: ${error.response.status} - ${error.response.data}`);
      }
      throw error;
    }
  }
